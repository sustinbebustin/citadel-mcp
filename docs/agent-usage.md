...
- **30-second async timeout.** If your function (or any tool call inside it) is still pending after 30s, you get an `Execution timed out after 30000ms` error. Long fetches are vanishingly rare here, but don't write infinite loops.
- **No filesystem, no `process`, no spawn.** Treat the sandbox as a JS-only computation environment. The host has Node capabilities, but you should not rely on them — the contract is the `codemode` SDK plus standard JS built-ins (`fetch` is *not* exposed; use the SDK).
